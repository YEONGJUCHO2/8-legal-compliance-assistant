...
- [ ] Step 8: Add verification tests
  - Notes: cover happy path, missing article, disagreement, stale marking, in-force downgrade, and timeout-to-`verification_pending` behavior.
...
- Integration: compare seeded local citations against mocked MCP responses and confirm the returned citations are normalized and properly flagged under both normal and capped-parallel verification.
...
- [ ] Verification runs in capped parallelism, avoids N+1 metadata fetches, and downgrades before the platform timeout budget is exhausted
